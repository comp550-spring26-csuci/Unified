...
export default function BusinessOpportunities() {
  ...
  const communitiesQ = useListCommunitiesQuery(undefined, {
    skip: !userId || !isBusinessOwner,
  });
  const oppQ = useBusinessOpportunitiesQuery(applied, {
    skip: !userId || !isBusinessOwner,
  });
    ...
    return (
      <Box p={3}>
        <Typography color="text.secondary">
          Sign in to browse business opportunities.
        </Typography>
      </Box>
    );
  }
  ...
  if (!isBusinessOwner) {
    return (
      <Box p={3}>
        <Typography color="text.secondary">
          This page is available only to Business Owner accounts.
        </Typography>
      </Box>
    );
  }
  ...
  return (
    <Box p={3}>
      ...
      <Typography variant="body2" color="text.secondary" mb={3}>
        Upcoming events that are requesting your business category. Search by
        title, business requirements, venue, or narrow by date and community.
      </Typography>
      ...
      >
        <Stack spacing={2}>
          <TextField
            ...
            placeholder="Title, venue, category, or business requirements..."
            InputProps={{
              startAdornment: (
      ...
      {oppQ.isLoading ? (
        ...
        <CommunityEventsList
          ...
          emptyMessage="No matching business opportunities are available right now."
        />
      )}
    </Box>
  );
