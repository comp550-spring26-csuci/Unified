...
async function updateProfile(req, res) {
  ...
  if (data.mailingAddress !== undefined) {
    user.mailingAddress = data.mailingAddress;
  }
  if (data.interests !== undefined) {
    user.interests = data.interests.slice(0, 20);
  }

  // Profile image path must match uploads/avatars folder
  if (req.file) {
...
async function forgotPassword(req, res) {
  ...
  if (!sent.ok) {
    ...
    return res.status(503).json({
      message:
        'Email could not be sent. Check server logs and SMTP settings, then try again.',
      ...(detail ? { detail } : {}),
    });
  }
